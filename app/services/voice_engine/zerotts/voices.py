...
def load_voice(voices_root: str | Path, name: str,
    ...
    if not npz.exists():
        ...
        raise FileNotFoundError(f'no voice {name!r} in {root} '
    ...
    if stored_q != emb.shape[1]:
        raise ValueError(f'voice {name!r} is inconsistent: n_voice_queries={stored_q} '
                         f'but voice_emb has {emb.shape[1]} queries.')
    if expect_queries is not None and stored_q != expect_queries:
        raise ValueError(f'voice {name!r} was built with n_voice_queries={stored_q}, '
                         f'but this model uses {expect_queries}. It belongs to different weights.')
